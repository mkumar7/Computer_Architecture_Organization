...
//            "MemoryToWriteback"};
        ...
        public void compute()
        {
            ...
            int dest_reg = ins.getOper0().getRegisterNumber();
            
            
            while (globals.getPropertyBoolean("decode_squash"))
            {
...
//                globals.setClockedProperty("branch_state_decode", GlobalData.BRANCH_STATE_NULL);
          ...
          //  }
            ...
            if(ins.getOper0().isRegister() && ins.getOpcode().oper0IsSource()  )
            {
            	ins.getOper0().rename(rat[ins.getOper0().getRegisterNumber()]);
            }
            if(ins.getSrc1().isRegister())
            {
            	ins.getSrc1().rename(rat[ins.getSrc1().getRegisterNumber()]);
            }
            
            if(ins.getSrc2().isRegister())
            {
            	ins.getSrc2().rename(rat[ins.getSrc2().getRegisterNumber()]);
            }
...
//                            Logger.out.println("Stall JMP wants oper0 R" + oper0.getRegisterNumber());
                    ...
                    while (!d2e_output.canAcceptWork()) 
                    	return; 
                    
                    
                    if(ins.getOpcode().needsWriteback())
                    {
                    	int temp_register = rat[dest_reg];
                    	register.markRenamed(temp_register, true);
                    	rat[dest_reg] = Unused_M_reg;
                    	register.changeFlags(Unused_M_reg, register.SET_USED | register.SET_INVALID, register.CLEAR_FLOAT | register.CLEAR_RENAMED);
                    	ins.getOper0().rename(Unused_M_reg);
                    }
...
//                    return;
            ...
            
            {
                ...
                while (!input.hasProperty(propname)) 
                {
